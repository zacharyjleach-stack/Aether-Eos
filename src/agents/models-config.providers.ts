...
/**
 * Detect whether a provider targets a local Ollama instance.
 * Matches the provider key "ollama" (or keys containing "ollama", e.g. "local_ollama")
 * as well as any provider whose baseUrl points to Ollama's default port (11434).
 */
function isOllamaProvider(providerKey: string, baseUrl?: string): boolean {
  if (providerKey.toLowerCase().includes("ollama")) {
    return true;
  }
  if (baseUrl) {
    try {
...
/**
 * Inject `params: { streaming: false }` on every model definition in an
 * Ollama provider that does not already carry the flag. This works around
 * pi-ai SDK issue #1205 where streaming from Ollama can silently fail.
 *
 * The implicit Ollama provider (auto-discovered via `discoverOllamaModels`)
 * already sets this flag, but explicitly-configured providers (e.g. the
 * `local_ollama` provider generated by the setup scripts) do not.
 */
function normalizeOllamaProvider(provider: ProviderConfig): ProviderConfig {
  let mutated = false;
  const models = provider.models.map((model) => {
    // The TypeScript type does not include `params`, but the JSON
    // serialisation carries it through to models.json where the SDK reads it.
    const existing = (model as ModelDefinitionConfig & { params?: Record<string, unknown> }).params;
    if (existing && typeof existing.streaming === "boolean") {
      return model; // Already set — respect the explicit value
    }
    mutated = true;
    return {
      ...model,
...
export function normalizeProviders(params: {
  ...
  for (const [key, provider] of Object.entries(providers)) {
    ...
    // Disable streaming for Ollama-compatible providers to avoid SDK issue #1205
    if (isOllamaProvider(normalizedKey, normalizedProvider.baseUrl)) {
      const ollamaNormalized = normalizeOllamaProvider(normalizedProvider);
      if (ollamaNormalized !== normalizedProvider) {
        mutated = true;
      }
      normalizedProvider = ollamaNormalized;
    }
